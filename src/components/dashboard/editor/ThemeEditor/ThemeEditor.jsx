...
export default function ThemeEditor() {
    ...
    useEffect(() => {
        const fetchThemes = async () => {
            if (!userId) return;
            try {
                const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/themes?userId=${userId}`);
            ...
            <SheetContent>
                <SheetHeader>
                    ...
                    <SheetDescription asChild>
                        <div suppressHydrationWarning className="mt-4 px-5 h-[30%] overflow-x-hidden overflow-y-auto mx-auto gap-4 grid grid-cols-2 items-center">
                            {loading ? (
